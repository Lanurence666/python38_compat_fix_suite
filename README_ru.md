...
| **FastAPI** | 0.115.14 (последний main) | Скомпилирован и протестирован на Python 3.8 | [fastapi_backport_py38](https://github.com/Lanurence666/fastapi_backport_py38) |
...
### Результаты тестирования FastAPI (Python 3.8)

Последняя версия **FastAPI 0.115.14** была портирована и проверена на Python 3.8.10:

**✅ Проверенная функциональность (46/46 основных тестов пройдено):**
- Создание приложений и маршрутизация (GET, POST, PUT, DELETE)
- Параметры пути / запроса с валидацией
- Тела запросов с моделями Pydantic v2 (включая вложенные модели, типы Union)
- Внедрение зависимостей (простое, на основе классов, вложенное)
- APIRouter с префиксами и тегами
- Генерация схемы OpenAPI (включая схемы моделей Pydantic)
- `jsonable_encoder` (модели Pydantic, словари, datetime)
- Обработка `HTTPException` (с пользовательскими заголовками)
- Множество классов ответов (JSON, HTML, PlainText, Redirect)
- Настройка кодов состояния
- Параметры форм и загрузка файлов
- Параметры Header и Cookie
- Фоновые задачи
- Промежуточное ПО CORS
- Полное CRUD-приложение с несколькими маршрутизаторами

**Сводка тестового набора:**
| Метрика | Количество |
|---------|-----------|
| Пройдено | 747 |
| Не пройдено | 869 |
...
> Большинство сбоев в тестовых файлах, использующих `type[Model] | None` в качестве типов полей моделей Pydantic. Основная библиотека FastAPI работает корректно.

**🔧 Дополнительные ручные исправления (помимо автоматического скрипта):**
- Подписные типы `collections.abc` (`Callable`, `Awaitable`, `Generator`, `Coroutine`, `AsyncIterator`, `AsyncGenerator`) → перенос в импорт модуля `typing` (теперь автоматически исправляется в v2)
- `contextlib.AbstractAsyncContextManager[T]` → `typing.AsyncContextManager[T]` (теперь автоматически исправляется в v2)
- Подписка `type["X"]` во время выполнения → `Type["X"]` из `typing` (теперь автоматически исправляется в v2)
- `inspect.signature(eval_str=True)` → версия с проверкой `sys.version_info >= (3, 10)` (теперь автоматически исправляется в v2)
- Модуль `typing_inspection` → `try/except ImportError` с откатом (теперь автоматически исправляется в v2)
- `types.GenericAlias` / `types.UnionType` → проверки `hasattr` (теперь автоматически исправляется в v2)
- Зависимость `eval_type_backport` для вычисления типов `X | Y` во время выполнения в Pydantic v2
- Версия Starlette зафиксирована `>=0.27.0,<0.36.0` (последний совместимый с Python 3.8 диапазон)
- Сжатые однострочные мульти-импорты → разделение на многострочный формат
...
| 58 | Импорты с обратным слешем (`from X import \`) | — | Преобразование в многострочные импорты со скобками для предотвращения синтаксических ошибок при модификации импортов другими исправлениями |
| 59 | Конфликт `from X import *` + `from X import (конкретные)` | — | Удаление `import *`, когда существуют конкретные импорты из того же модуля — предотвращает синтаксические ошибки после слияния импортов |
| 60 | Недопустимые escape-последовательности (`'\?'`, `'\d'` и т.д.) | 3.12+ предупреждение | Удвоение обратного слеша в недопустимых escape-последовательностях внутри строковых литералов |
| 61 | Пропущенный оператор перед скобкой (`x**2(y)`) | — | Вставка пропущенного оператора `*` между степенными выражениями и скобками |
| 62 | Прямые ссылки `importlib.metadata.xxx` | 3.9+ | Замена `importlib.metadata.version()` и т.д. на `importlib_metadata.version()` после добавления совместимого импорта |
| 63 | Импорт `import zoneinfo` на уровне функции | 3.9+ | `try/except` с откатом на `backports.zoneinfo` с правильным отступом |
| 64 | Формат license в `pyproject.toml` | — | Преобразование `license = "XXX"` → `license = {text = "XXX"}` для совместимости со старыми setuptools; удаление `license-files`; понижение `setuptools>=69` → `setuptools>=64` |
| 65 | Файл журнала отката | — | При откате файла из-за синтаксических ошибок, запись деталей в `python38-pythonfix-log.txt` для ручной проверки |
| 66 | `inspect.signature(eval_str=True)` | 3.10+ | Добавление проверки версии `sys.version_info >= (3, 10)`: использовать `eval_str=True` на 3.10+, опускать на 3.8/3.9 |
| 67 | Импорт модуля `typing_inspection` | 3.9+ | Обёртка `from typing_inspection import ...` в `try/except ImportError` с резервными реализациями |
| 68 | Подписка `contextlib.AbstractAsyncContextManager[T]` | 3.9+ | Замена на `typing.AsyncContextManager[T]`; также обрабатывает `AbstractContextManager[T]` → `typing.ContextManager[T]` |
| 69 | Подписка `type["X"]` во время выполнения | 3.9+ | Замена `type["X"]` на `Type["X"]` из `typing` в контексте выполнения (не в аннотациях); также обрабатывает `type[X] \| None` → `Optional[Type[X]]` |
| 70 | Отсутствующий импорт `partial` | — | Обнаружение использования `partial()` без `from functools import partial` и автоматическое добавление импорта |
| 71 | Автоматическое добавление `from __future__ import annotations` | — | Защитная сеть: обнаружение файлов, использующих синтаксис `X \| Y` или `list[int]` без `from __future__ import annotations`, и автоматическое добавление |
| 72 | Расширенные подписные типы `collections.abc` | 3.9+ | Обработка `Awaitable`, `Generator`, `Coroutine`, `AsyncIterator`, `AsyncIterable`, `AsyncGenerator` в дополнение к существующим `Callable`, `Iterator` и т.д. — перенос из `collections.abc` в импорт `typing` |
