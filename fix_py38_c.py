#!/usr/bin/env python3
"""
Python 3.8 Compatibility Fix Script - C/C++ Source
====================================================
Automatically fixes Python 3.9+ C API usage for Python 3.8 compatibility.

Usage:
  python fix_py38_c.py [source_directory]
...
def update_existing_pythoncapi_compat_h(root, project_config=None, skip_site_packages=True):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        static_fixed = fix_pythoncapi_compat_static_conflicts(existing_content)
        static_modified = (static_fixed != existing_content)

        missing = check_pythoncapi_compat_completeness(filepath)
        relpath = os.path.relpath(filepath, root)

        if not missing and not static_modified:
            continue

        if missing:
            from i18n import t
            print(t("missing_compat", relpath=relpath, count=len(missing), apis=', '.join(missing[:5]) + ('...' if len(missing) > 5 else '')))
        if static_modified:
            from i18n import t
            print(t("static_conflict", relpath=relpath))

        if is_project_compat and not static_modified:
            from i18n import t
            print(t("skip_project_compat"))
            print(t("manual_add_compat"))
            continue

        if static_modified and not missing:
            with open(filepath, "w", encoding="utf-8") as f:
                f.write(static_fixed)
            updated.append(relpath)
            from i18n import t
            print(t("fixed_static", relpath=relpath))
            continue
        ...
        updated.append(relpath)
        from i18n import t
        print(t("updated_compat", relpath=relpath))
...
def deploy_pythoncapi_compat_h(root, files_needing_compat, project_config=None, skip_site_packages=True):
    if project_config:
        ...
        if os.path.isfile(compat_h_path):
            ...
            if missing:
                from i18n import t
                print(t("project_missing_compat", count=len(missing)))
                print(f"  {', '.join(missing[:10])}{'...' if len(missing) > 10 else ''}")
            else:
                from i18n import t
                print(t("project_compat_ok"))
            return []
        else:
...
def fix_pythoncapi_compat_static_conflicts(content):
    if 'pythoncapi_compat' not in content and 'PYTHONCAPI_COMPAT' not in content:
        return content
    ...
    PY38_SYSTEM_HEADERS = {
        ...
        'PyModule_AddFunctions': {
            ...
            'new_version': '#if PY_VERSION_HEX < 0x03080000',
            'comment': '// Python 3.8 already provides PyModule_AddFunctions in modsupport.h',
        },
        'PyInterpreterState_GetDict': {
            'version_check': r'#if\s+PY_VERSION_HEX\s*<\s*0x030C00A2',
            'new_version': '#if PY_VERSION_HEX < 0x03080000',
            'comment': '// Python 3.8 already provides PyInterpreterState_GetDict in pystate.h',
        },
        'PyErr_GetExcInfo': {
    ...
    while i < len(lines):
        ...
        for api_name, info in PY38_SYSTEM_HEADERS.items():
            if re.match(info['version_check'], line.strip()):
                func_found = False
                for j in range(i + 1, min(i + 50, len(lines))):
                    if api_name in lines[j] and ('static inline' in lines[j] or 'static' in lines[j]):
                        func_found = True
                        break
                    if lines[j].strip().startswith('#if') or lines[j].strip().startswith('#endif'):
                        if j > i + 1:
                            break

                if func_found:
                    new_lines.append(info['comment'])
                    new_lines.append(info['new_version'])
                    modified = True
                    replaced = True
                    i += 1
                    break
...
def process_c_file(filepath):
    ...
    content = fix_pythoncapi_compat_static_conflicts(content)
...
def _check_upstream_compat_coverage():
    ...
    from i18n import t
    print(t("upstream_apis", count=len(upstream_apis)))
    print(t("combined_apis", count=len(combined_apis)))
    print(t("extra_apis", count=len(extra_only)))
    ...
    for i, line in enumerate(lines):
        ...
        if m:
            ...
            if name in func_defs:
                from i18n import t
                print(t("duplicate_warning", name=name, line1=func_defs[name], line2=i+1))
            else:
                func_defs[name] = i + 1
...
def main():
    ...
    if project_config:
        print(t("detected_project", name=project_name))
        print(t("project_include", path=project_config['include_line']))
        print()

    print(t("step_scan_c"))
    c_files = find_c_files(root, skip_site_packages=skip_site)
    print(t("found_c_files", count=len(c_files)))

    print()
    print(t("step_detect_c"))
    ...
    print(t("files_need_compat", count=len(files_needing_compat)))

    print()
    print(t("step_check_compat_h"))
    updated_compat = update_existing_pythoncapi_compat_h(root, project_config, skip_site_packages=skip_site)
    if updated_compat:
        print(t("modified_count", count=len(updated_compat)))
    else:
        print(t("all_compat_ok"))

    print()
    print(t("step_deploy_compat"))
    deployed = deploy_pythoncapi_compat_h(root, files_needing_compat, project_config, skip_site_packages=skip_site)
    if deployed:
        print(t("deployed_compat", count=len(deployed)))
        for d in deployed:
            print(f"    - {d}")
    else:
        print(t("compat_exists"))

    print()
    print(t("step_add_include"))
    include_count = 0
    for filepath in files_needing_compat:
        if add_pythoncapi_include(filepath, project_config):
            include_count += 1
            relpath = os.path.relpath(filepath, root)
            print(t("add_include", relpath=relpath))

    print(t("include_modified", count=include_count))

    print()
    print(t("step_fix_other_c"))
    other_fixed = 0
    for filepath in c_files:
        try:
            if process_c_file(filepath):
                other_fixed += 1
                relpath = os.path.relpath(filepath, root)
                print(t("fix_file", relpath=relpath))
        except Exception as e:
            relpath = os.path.relpath(filepath, root)
            print(t("error_file", relpath=relpath, error=e))

    if other_fixed:
        print(t("c_fixed_count", count=other_fixed))
    else:
        print(t("no_extra_fix"))

    print()
    print(t("step_verify_c"))
    ...
    for pattern, name in api_checks:
        ...
        if files_without_compat:
            print(t("api_unprotected", name=name, count=len(files_without_compat)))
            all_clean = False
        else:
            print(t("api_all_ok", name=name))
    ...
    if meth_method_count > 0:
        print(t("meth_method_remaining", count=meth_method_count))
        all_clean = False
    else:
        print(t("meth_method_ok"))
    ...
    for filepath in c_files_after:
        ...
        for idx, line in enumerate(lines):
            ...
            if not protected:
                ...
                relpath = os.path.relpath(filepath, root).replace("\\", "/")
                print(t("gil_api_unprotected", relpath=relpath, line=idx+1))
    if gil_unprotected_count > 0:
        print(t("gil_unprotected", count=gil_unprotected_count))
        all_clean = False
    else:
        print(t("gil_all_ok"))
