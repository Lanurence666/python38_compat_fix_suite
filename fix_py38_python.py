#!/usr/bin/env python3
"""
Python 3.8 Compatibility Fix Script - Python Source
====================================================
Automatically fixes Python 3.9+ syntax and APIs for Python 3.8 compatibility.

Usage:
  python fix_py38_python.py [source_directory]
...
def _wrap_decorator_block_with_hasattr(lines, start_idx, attr_name):
    indent = re.match(r'(\s*)', lines[start_idx]).group(1)
    inner_indent = indent + '    '
    block_start = start_idx
    while block_start < len(lines) and lines[block_start].strip().startswith('@'):
        block_start += 1
    if block_start >= len(lines):
        return lines, False
    if not (lines[block_start].strip().startswith('class ') or lines[block_start].strip().startswith('def ')):
        return lines, False
    first_line_indent = re.match(r'(\s*)', lines[block_start]).group(1)
    ...
    while block_end < len(lines):
        ...
        if line_stripped == '':
            block_end += 1
            continue
        line_indent = re.match(r'(\s*)', lines[block_end]).group(1)
        if len(line_indent) <= len(first_line_indent) and line_stripped:
            break
        block_end += 1
    new_lines = lines[:start_idx]
    new_lines.append(f"{indent}if hasattr(math, '{attr_name}'):")
    for i in range(start_idx, block_end):
        new_lines.append(inner_indent + lines[i])
...
def fix_math_nextafter_ulp(content):
    ...
    if not needs_nextafter and not needs_ulp:
        if '_math_nextafter_compat' in content and 'def test_nextafter' in content:
            lines = content.split("\n")
            new_lines = []
            for i, line in enumerate(lines):
                new_lines.append(line)
                stripped = line.strip()
                if stripped.startswith('def test_nextafter') and 'self' in stripped:
                    has_skip = False
                    for j in range(max(0, i - 3), i):
                        if 'skipUnless' in lines[j] and 'nextafter' in lines[j]:
                            has_skip = True
                            break
                    if not has_skip:
                        indent = re.match(r'(\s*)', line).group(1)
                        new_lines.insert(len(new_lines) - 1,
                            f'{indent}@unittest.skipUnless(hasattr(math, "nextafter"), "needs Python 3.9+")')
            return "\n".join(new_lines)
        return content

    if needs_nextafter and ("def _math_nextafter_compat" in content or "_math_nextafter_compat" in content):
        needs_nextafter_call = bool(re.search(r'\bmath\.nextafter\s*\(', content))
        needs_nextafter_decorator = bool(re.search(r'@\w+\(math\.nextafter', content))
        needs_nextafter = needs_nextafter_call or needs_nextafter_decorator
    if needs_ulp and ("def _math_ulp_compat" in content or "_math_ulp_compat" in content):
        needs_ulp = bool(re.search(r'\bmath\.ulp\s*\(', content))
    ...
    if needs_nextafter:
        decorator_indices = []
        for i, line in enumerate(lines):
            if re.search(r'@\w+\(math\.nextafter', line):
                already_wrapped = False
                for j in range(max(0, i - 5), i):
                    if 'hasattr(math' in lines[j] and 'nextafter' in lines[j]:
        ...
        if decorator_indices:
            for idx in reversed(decorator_indices):
                lines, ok = _wrap_decorator_block_with_hasattr(lines, idx, 'nextafter')
                if not ok:
                    for j in range(idx, min(idx + 20, len(lines))):
                        if 'math.nextafter' in lines[j]:
                            lines[j] = lines[j].replace('math.nextafter(', '_math_nextafter_compat(')
                            needs_nextafter_compat = True
                            break

    new_lines = []
    for line in lines:
        if needs_nextafter and re.search(r'\bmath\.nextafter\s*\(', line) and '_math_nextafter_compat' not in line and 'hasattr' not in line:
            needs_nextafter_compat = True
            line = re.sub(r'\bmath\.nextafter\s*\(', '_math_nextafter_compat(', line)
        if needs_ulp and "math.ulp(" in line and "_math_ulp_compat" not in line:
    ...
    if needs_nextafter_compat or any('math.nextafter' in l or '_math_nextafter_compat' in l for l in new_lines):
        final_lines = []
        for i, line in enumerate(new_lines):
            ...
            if stripped.startswith('def test_nextafter') and 'self' in stripped:
                has_skip = False
                for j in range(max(0, i - 3), i):
                    if 'skipUnless' in new_lines[j] and 'nextafter' in new_lines[j]:
                        has_skip = True
                        break
                if not has_skip:
                    indent = re.match(r'(\s*)', line).group(1)
...
def fix_cached_property_import(content):
    if '@cached_property' not in content and 'cached_property' not in content:
        return content
    if re.search(r'from\s+functools\s+import\s+.*\bcached_property\b', content):
        return content
    if re.search(r'try:\s*\n\s*from\s+functools\s+import\s+.*\bcached_property\b', content):
        return content
    if 'import cached_property' in content:
        return content
    if 'except ImportError' in content and 'cached_property' in content:
        return content
    if not re.search(r'\bcached_property\b', content):
        return content

    if '@cached_property' not in content and not re.search(r'\bcached_property\s*[\(\[]', content):
        return content
    ...
    for line in lines:
        ...
        if m:
            ...
            if module in seen_from:
                ...
                pm = re.match(r'^from\s+[\w.]+\s+import\s+(.+)$', prev_line)
                if pm:
                    prev_items = [x.strip() for x in pm.group(1).split(',') if x.strip()]
                    merged = list(dict.fromkeys(prev_items + items))
                    new_lines[prev_idx] = f"from {module} import {', '.join(merged)}"
                    continue
            else:
                seen_from[module] = len(new_lines)
                new_lines.append(line)
                continue

        m2 = re.match(r'^import\s+([\w.]+)(\s+as\s+\w+)?\s*$', line)
        if m2:
            module = m2.group(1)
            alias = m2.group(2) or ''
            key = module + alias
...
def fix_array_api_compat_typing(content):
    if 'array_api_compat' not in content:
        return content
    ...
    for pattern, cap_name in PEP585_REPLACEMENTS:
        if re.search(pattern, content):
            def _make_replacer(name):
                def _replacer(m):
                    inner = m.group(1)
                    return f'{name}[{inner}]'
                return _replacer
            content = re.sub(pattern, _make_replacer(cap_name), content)

    needs_typing_import = False
    for cap_name in ['Tuple', 'List', 'Dict', 'Set', 'FrozenSet', 'Type']:
        if f'{cap_name}[' in content:
    ...
    if needs_typing_import:
        ...
        m = re.search(r'^from typing import (.+)$', content, re.MULTILINE)
        if m:
            existing_typing = {x.strip().split(' as ')[0].split('[')[0].strip() for x in m.group(1).split(',')}

        needed = set()
        if 'Tuple[' in content and 'Tuple' not in existing_typing:
            needed.add('Tuple')
        if 'List[' in content and 'List' not in existing_typing:
            needed.add('List')
        if 'Dict[' in content and 'Dict' not in existing_typing:
            needed.add('Dict')
        if 'Set[' in content and 'Set' not in existing_typing:
            needed.add('Set')
        if 'FrozenSet[' in content and 'FrozenSet' not in existing_typing:
            needed.add('FrozenSet')
        if 'Type[' in content and 'Type' not in existing_typing:
            needed.add('Type')
        ...
        if needed:
            if m:
                ...
                for n in sorted(needed):
                    if n not in imports:
                        imports.append(n)
                content = content.replace(old_line, f"from typing import {', '.join(imports)}", 1)
            else:
                lines = content.split('\n')
                insert_pos = 0
                for i, line in enumerate(lines):
...
def main():
    ...
    if not os.path.isdir(root):
        print(t("error_not_dir", root=root))
        sys.exit(1)
    ...
    print(t("py_title"))
    print("=" * 60)
    print(t("work_dir", root=root))
    if skip_site:
        print(t("skip_site_yes"))
    else:
        print(t("skip_site_no"))
    print()

    print(t("step_fix_config"))
    config_modified = []
    ...
    if config_modified:
        print(t("modified", items=', '.join(config_modified)))
    else:
        print(t("no_change"))

    print()
    print(t("step_scan_py"))
    py_files = find_py_files(root, skip_site_packages=skip_site)
    print(t("found_py_files", count=len(py_files)))
    ...
    for i, filepath in enumerate(py_files):
        ...
        try:
            ...
            relpath = os.path.relpath(filepath, root)
            print(t("error_file", relpath=relpath, error=e))

    print(t("modified_count", count=modified_count))
    if modified_count <= 50:
        for f in modified_files:
            print(f"    - {f}")

    print()
    print(t("step_verify_py"))
    ...
    if syntax_errors:
        print(t("syntax_errors", count=len(syntax_errors)))
        for relpath, err in syntax_errors:
            print(f"      {relpath}: {err}")
        all_clean = False
    else:
        print(t("syntax_ok", count=len(py_files_after)))
    ...
    checks = [
        ...
        (r"^class\s+\w+\[", "PEP 695 generic class (3.12+)"),
        (r"^\s*@lambda\b", "PEP 614 lambda decorator (3.9+)"),
        ...
        (r"\bast\.unparse\s*\(", "ast.unparse (3.9+)"),
        (r"\brandom\.randbytes\b", "random.randbytes (3.9+)"),
        (r"^from collections import .*\b(Mapping|MutableMapping|Iterable|Iterator|Sequence|MutableSequence|Set|MutableSet|Callable|Container|Hashable|Sized|Collection|Reversible)\b", "collections.XXX -> collections.abc.XXX (3.9+)"),
        (r"\bzip\s*\([^)]*strict\s*=\s*True", "zip(strict=True) (3.10+)"),
        (r"(?<!\w)([a-zA-Z_]\w*)\.bit_count\(\)", "int.bit_count() (3.10+)"),
        (r"\baiter\s*\(", "aiter() (3.10+)"),
        (r"\banext\s*\(", "anext() (3.10+)"),
        ...
        (r"\bmath\.cbrt\(", "math.cbrt() (3.11+)"),
        (r"\bdatetime\.UTC\b", "datetime.UTC (3.11+)"),
        (r"\bcontextlib\.chdir\(", "contextlib.chdir() (3.11+)"),
        ...
        (r"\bcopy\.replace\(", "copy.replace() (3.13+)"),
        (r"^from (aifc|audioop|cgi|cgitb|chunk|crypt|imghdr|mailcap|msilib|nis|nntplib|ossaudiodev|pipes|sndhdr|spwd|sunau|telnetlib|uu|xdrlib|lib2to3)\b", "PEP 594 removed modules (3.13)"),
        (r"\bbase64\.z85", "base64.z85encode/decode() (3.13+)"),
        (r"\bfrom compression import", "compression module (3.14+)"),
        (r"\bfrom concurrent\.interpreters import", "concurrent.interpreters (3.14+)"),
        (r"\bimport annotationlib\b(?!.*(?:try:|except|version_info))", "annotationlib (3.14+)"),
        (r"\bfrozendict\b", "frozendict (3.15+)"),
    ...
    for pattern, desc in checks:
        ...
        for filepath in py_files_after:
            ...
            if "from __future__ import annotations" in file_content and desc in ("isinstance(x, A|B)", "issubclass(x, A|B)"):
                continue
            if "tomli" in file_content and "tomllib" in desc:
                continue
            ...
            if "nextafter" in desc:
                ...
                for m in re.finditer(pattern, file_content, re.MULTILINE):
                    ...
                    line = file_content[line_start:start + len(m.group())]
                    if 'hasattr(math' in line:
                        continue
                    if '_math_nextafter_compat' in before[max(0, start - 30):start + len(m.group())]:
                        continue
                    prev_lines = before.rsplit('\n', 5)[-5:]
                    if any('hasattr(math' in pl and 'nextafter' in pl for pl in prev_lines):
                        continue
                    filtered += 1
                count += filtered
                continue
        ...
        if count > 0:
            print(t("residual", desc=desc, count=count))
            all_clean = False
        else:
            print(t("all_fixed", desc=desc))
